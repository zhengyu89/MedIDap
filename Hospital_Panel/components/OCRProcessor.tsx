...
export const OCRProcessor: React.FC<OCRProcessorProps> = ({ onProcessingComplete, onCancel }) => {
  ...
  const processData = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err instanceof Error ? err.message : "Failed to process data. Please try again.");
    } finally {
      setProcessing(false);
    }
  };
        ...
        <div className="w-full md:w-1/2 p-8 border-r border-gray-200 flex flex-col">
          ...
          <div className="flex p-1 bg-gray-100 rounded-lg mb-6">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-sm font-medium transition-all ${mode === 'upload' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-md text-sm font-medium transition-all ${mode === 'manual' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
            >
          ...
          <div className="mt-6 flex gap-3">
            ...
            {!result ? (
              <button
                ...
                className={`flex-1 px-4 py-3 rounded-xl flex items-center justify-center gap-2 font-medium text-white transition-all ${isAnalyzeDisabled ? 'bg-gray-400 cursor-not-allowed' : 'bg-[#0060c4] hover:bg-blue-700 shadow-md'
                  }`}
              >
                {processing ? (
                  <>
